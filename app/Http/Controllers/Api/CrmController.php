...
class CrmController extends Controller
{
    private function autorizarGestao(Request $request): void
    {
        ...
        if (!$usuario) abort(401, 'Usuário não autenticado.');
        if (($usuario->is_master ?? false) || ($usuario->is_admin ?? false)) return;
        ...
        if (!$autorizado) abort(403, 'Ação restrita a Administradores e Gestores Comerciais.');
    }
    ...
    public function destroyEtapa(Request $request, string $id): JsonResponse
    {
        ...
        if (CrmOportunidade::where('etapa_id', $etapa->id)->exists()) {
    ...
    public function reordenarEtapas(Request $request, string $pipelineId): JsonResponse
    {
        ...
        DB::transaction(function () use ($validated, $pipeline) {
            foreach ($validated['etapas'] as $item) {
                CrmFunilEtapa::where('id', $item['id'])->where('funil_id', $pipeline->id)->update(['ordem_exibicao' => $item['ordem_exibicao']]);
            }
        });

        return response()->json(['data' => ['message' => 'Ordem das etapas atualizada.']]);
    }
    ...
    public function board(Request $request): JsonResponse
    {
            ...
            ->with(['etapas' => function ($queryEtapa) use ($statusFiltro, $vendedorId, $search, $tenantId) {
                $queryEtapa->orderBy('ordem_exibicao', 'asc')
                    ->with(['oportunidades' => function ($q) use ($statusFiltro, $vendedorId, $search, $tenantId) {
                        ...
                        if ($statusFiltro !== 'TODOS') $q->where('status', $statusFiltro);
                        if (!empty($vendedorId)) $q->where('vendedor_id', $vendedorId);
                        if (!empty($search)) {
        ...
        return response()->json([
            'data' => [
                ...
                'produtos' => $produtos,
                'permissoes' => ['pode_gerenciar_pipeline' => $isGestor]
            ]
        ]);
    }
    ...
    {
        $validated = $request->validate(['etapa_id_destino' => 'required|uuid|exists:crm_funil_etapas,id']);
        $tenantId = $request->user()->tenant_id;
        ...
        return response()->json(['data' => ['message' => 'Oportunidade movida.', 'oportunidade' => $oportunidade]]);
    }
    ...
    public function storeOportunidade(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'itens.*.produto_id' => 'nullable|uuid|exists:cad_produtos,id',
        ...
        $oportunidade = DB::transaction(function () use ($validated, $tenantId, $etapa, $request) {
            $totalItens = 0;
            if (!empty($validated['itens'])) {
                foreach ($validated['itens'] as $it) {
                    $totalItens += (float)$it['quantidade'] * (float)$it['valor_unitario'];
                }
            }

            $valorFinal = $totalItens > 0 ? $totalItens : (float)($validated['valor_estimado'] ?? 0);

            $op = CrmOportunidade::create([
                'id' => (string) Str::uuid(),
                'tenant_id' => $tenantId,
            ...
            if (!empty($validated['itens'])) {
                foreach ($validated['itens'] as $it) {
                    CrmOportunidadeItem::create([
                        ...
                        'valor_total' => (float)$it['quantidade'] * (float)$it['valor_unitario'],
                    ]);
                }
            }
        ...
        return response()->json(['data' => $oportunidade->load('itens')], 201);
    }
    ...
    public function adicionarItemOportunidade(Request $request, string $id): JsonResponse
    {
        ...
        $validated = $request->validate([
            'produto_id' => 'nullable|uuid|exists:cad_produtos,id',
    ...
    public function converterParaOrcamento(Request $request, string $id): JsonResponse
    {
        ...
        try {
            return DB::transaction(function () use ($oportunidade, $tenantId, $request) {
                ...
                $empresaId = $request->user()->empresa_padrao_id ?? Empresa::where('tenant_id', $tenantId)->first()?->id ?? Empresa::first()?->id;
                $depositoId = Deposito::where('tenant_id', $tenantId)->where('is_padrao', true)->first()?->id ?? Deposito::where('tenant_id', $tenantId)->first()?->id ?? Deposito::first()?->id;
                ...
                $oportunidade->update(['status' => 'GANHO', 'data_fechamento' => now()]);
    ...
    public function storeMotivoPerda(Request $request): JsonResponse
    {
        ...
        $maxOrdem = TabelaDominio::where('tenant_id', $tenantId)->where('tipo_lista', 'CRM_MOTIVO_PERDA')->max('ordem_exibicao') ?? 0;
        $codigo = !empty($validated['codigo']) ? Str::slug($validated['codigo'], '_') : Str::slug($validated['nome'], '_');

        $motivo = TabelaDominio::create([
            'id' => (string) Str::uuid(),
            'tenant_id' => $tenantId,
            'tipo_lista' => 'CRM_MOTIVO_PERDA',
            'codigo' => strtoupper($codigo),
            'nome' => $validated['nome'],
            'cor_hex' => $validated['cor_hex'] ?? '#ef4444',
            'ordem_exibicao' => $maxOrdem + 1,
            'is_ativo' => true,
            'is_sistema' => false,
        ]);

        return response()->json(['data' => $motivo], 201);
    }

    public function updateMotivoPerda(Request $request, string $id): JsonResponse
    {
        $this->autorizarGestao($request);
        $tenantId = $request->user()->tenant_id;
        $motivo = TabelaDominio::where('tenant_id', $tenantId)->where('tipo_lista', 'CRM_MOTIVO_PERDA')->findOrFail($id);
        ...
        return response()->json(['data' => $motivo]);
    }

    public function destroyMotivoPerda(Request $request, string $id): JsonResponse
    {
        $this->autorizarGestao($request);
        $tenantId = $request->user()->tenant_id;
        $motivo = TabelaDominio::where('tenant_id', $tenantId)->where('tipo_lista', 'CRM_MOTIVO_PERDA')->findOrFail($id);

        if (CrmOportunidade::where('motivo_perda_id', $motivo->id)->exists()) {
            return response()->json(['error' => 'Não é possível excluir motivo de perda já vinculado a oportunidades.'], 422);
        }
    ...
    public function metricasAnaliticas(Request $request): JsonResponse
    {
        ...
        $query = CrmOportunidade::where('tenant_id', $tenantId);
        if (!empty($pipelineId)) {
            $query->where('funil_id', $pipelineId);
        }

        $totalGeral = (clone $query)->count();
        $totalGanhos = (clone $query)->where('status', 'GANHO')->count();
        $totalPerdidos = (clone $query)->where('status', 'PERDIDO')->count();
        $totalAbertos = (clone $query)->where('status', 'ABERTO')->count();

        $taxaConversao = ($totalGanhos + $totalPerdidos) > 0 ? round(($totalGanhos / ($totalGanhos + $totalPerdidos)) * 100, 1) : 0;
        $volumeTotalGanhos = (clone $query)->where('status', 'GANHO')->sum('valor_estimado');

        // Ranking Motivos de Perda
        $rankingPerdas = DB::table('crm_oportunidades')
            ->join('sis_tabelas_dominio', 'crm_oportunidades.motivo_perda_id', '=', 'sis_tabelas_dominio.id')
            ->where('crm_oportunidades.tenant_id', $tenantId)
            ->where('crm_oportunidades.status', 'PERDIDO')
            ->select('sis_tabelas_dominio.nome', 'sis_tabelas_dominio.cor_hex', DB::raw('count(crm_oportunidades.id) as total'))
            ->groupBy('sis_tabelas_dominio.nome', 'sis_tabelas_dominio.cor_hex')
            ->orderByDesc('total')
            ->get();

        // Conversão por Vendedor
        $desempenhoVendedores = DB::table('crm_oportunidades')
            ->join('users', 'crm_oportunidades.vendedor_id', '=', 'users.id')
            ->where('crm_oportunidades.tenant_id', $tenantId)
            ->select(
                'users.name as vendedor',
                DB::raw("count(case when crm_oportunidades.status = 'GANHO' then 1 end) as ganhos"),
                DB::raw("count(case when crm_oportunidades.status = 'PERDIDO' then 1 end) as perdidos"),
                DB::raw("sum(case when crm_oportunidades.status = 'GANHO' then crm_oportunidades.valor_estimado else 0 end) as valor_ganho")
            )
            ->groupBy('users.name')
            ->get();
        ...
        return response()->json([
            'data' => [
                ...
                'volume_total_ganhos' => (float)$volumeTotalGanhos,
                'ranking_perdas' => $rankingPerdas,
                'desempenho_vendedores' => $desempenhoVendedores,
            ]
        ]);
    }

    // --- WEBHOOK PÚBLICO DE ENTRADA (INBOUND) ---
    ...
    public function webhookCapturaLead(Request $request, string $token): JsonResponse
    {
        ...
        if (!$pipeline) return response()->json(['error' => 'Token de captação inválido ou inativo.'], 404);
        ...
        if (!$primeiraEtapa) return response()->json(['error' => 'Pipeline sem etapas configuradas.'], 500);
        ...
        return response()->json(['data' => ['message' => 'Lead capturado com sucesso!', 'oportunidade_id' => $oportunidade->id]], 201);
    }
