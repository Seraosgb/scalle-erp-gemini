...
export default function ConfiguracoesCrm() {
    ...
    const carregarDados = async (selecionarId = null) => {
        try {
            setLoading(true);
            const [resPipes, resMetricas] = await Promise.all([
            ...
            setPipelines(pipes);
            setMetricas(resMetricas.data?.data || null);
            ...
            if (alvoId) {
                ...
                setPipelineSelecionado(dataBoard?.pipeline || pipes[0]);
    ...
    const criarNovoFunil = async (e) => {
        ...
        try {
            ...
            avisarAtualizacao();
            await carregarDados(data?.data?.id);
        } catch (err) {
    ...
    const salvarEdicaoPipeline = async (e) => {
        ...
        try {
            ...
            alert(err.response?.data?.message || "Erro ao salvar funil.");
        } finally {
            setSalvando(false);
        }
    };
    ...
    if (loading && !metricas) {
        return (
            <div className="p-8 text-center text-slate-400">
                ...
                Carregando dados analíticos do CRM...
            </div>
        );
    }
    ...
    return (
        <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4">
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                        Métricas de conversão, multi-funis parametrizáveis e motivos de perda por tenant.
                    </p>
                </div>
                <button
                    type="button"
            ...
            {/* Aba 1: Métricas Analíticas */}
            {tabAtiva === 'metricas' && metricas && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                            ...
                            <p className="text-2xl font-bold text-indigo-400 mt-1">
                                {metricas.volume_total_ganhos.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </p>
                    ...
                    <div className="lg:col-span-4 bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                ...
                                className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg font-bold transition cursor-pointer"
                            >
                                + Novo Funil
                            </button>
                        </div>
                            ...
                            {pipelines.map(p => (
                                <div
                                    ...
                                    onClick={async () => {
                                        ...
                                        setPipelineSelecionado(res.data?.data?.pipeline || p);
                                        setEditandoPipeline(false);
                                    }}
                                ...
                                >
                                    ...
                                    {p.is_padrao && <span className="bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded text-[10px]">Padrão</span>}
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-8 bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-6">
                            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 mt-0.5">
                                        Token Webhook Inbound: <code className="bg-slate-950 px-2 py-0.5 rounded text-indigo-400 font-mono text-[11px]">{pipelineSelecionado.token_captacao}</code>
                                    </p>
                                </div>
                                <button
                                    type="button"
                            ...
                            {editandoPipeline && (
                                <form onSubmit={salvarEdicaoPipeline} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                                    ...
                                    <div className="flex justify-end gap-2">
                                        <button type="submit" disabled={salvando} className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 rounded text-xs font-bold">
                                            Salvar Alterações
                                        </button>
                                    </div>
                                </form>
                            ...
                            <div className="space-y-4">
                                    ...
                                    {(pipelineSelecionado.etapas || []).map((et, idx) => (
                                        <div key={et.id} className="flex justify-between items-center bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-bold text-slate-100">{idx + 1}. {et.nome}</span>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        setEtapaEmEdicao(et);
                                                        setFormEtapa({ nome: et.nome, probabilidade_fechamento: et.probabilidade_fechamento, cor_hex: et.cor_hex });
                                                    }}
                                                    className="text-slate-400 hover:text-white px-2 py-1 bg-slate-900 rounded border border-slate-800 cursor-pointer"
                                                >
                                                    Editar
                                                </button>
                                                <button
                                                    ...
                                                    className="text-rose-400 hover:text-rose-300 px-2 py-1 bg-rose-950/40 rounded border border-rose-900 cursor-pointer"
                                                >
                                                    Excluir
                                                </button>
                                            </div>
                                ...
                                <form onSubmit={salvarEtapa} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                                    <h5 className="text-xs font-bold text-slate-300">{etapaEmEdicao ? `Editar Etapa` : `+ Adicionar Nova Etapa`}</h5>
                                    ...
                                    <div className="flex justify-end gap-2">
                                        {etapaEmEdicao && (
                                            <button type="button" onClick={() => setEtapaEmEdicao(null)} className="px-3 py-1 text-xs text-slate-400">
                                                Cancelar
                                            </button>
                                        )}
                                        <button type="submit" disabled={salvando} className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded text-xs font-bold">
                                            {etapaEmEdicao ? 'Atualizar' : 'Cadastrar'}
                                        </button>
                                    </div>
                                </form>
                            </div>
            ...
            {tabAtiva === 'motivos_perda' && (
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-6">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">Cadastre, edite e personalize os motivos de descarte utilizados pela equipe comercial.</p>
                    </div>

                    <form onSubmit={salvarMotivoPerda} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                        <h4 className="text-xs font-bold text-slate-300">{motivoEmEdicao ? 'Editar Motivo de Perda' : '+ Novo Motivo de Perda'}</h4>
                        <div className="grid grid-cols-12 gap-3">
                        ...
                        <div className="flex justify-end gap-2">
                            {motivoEmEdicao && (
                                <button type="button" onClick={() => setMotivoEmEdicao(null)} className="px-3 py-1 text-xs text-slate-400">
                                    Cancelar
                                </button>
                            )}
                            <button type="submit" disabled={salvando} className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded text-xs font-bold">
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl text-slate-100 space-y-4">
                        <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                            ...
                            <button type="button" onClick={() => setModalNovoFunil(false)} className="text-slate-400 hover:text-white font-bold">✕</button>
                        </div>
                        <form onSubmit={criarNovoFunil} className="space-y-4">
                            <div>
                                <label className="block text-xs text-slate-300 mb-1">Nome do Funil *</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-xs text-slate-300 mb-1">Cor</label>
                                <input
                                    type="color"
                            ...
                            <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
                                <button type="button" onClick={() => setModalNovoFunil(false)} className="px-4 py-2 text-xs text-slate-400">Cancelar</button>
                                <button type="submit" disabled={salvando} className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-xl text-xs font-bold">
