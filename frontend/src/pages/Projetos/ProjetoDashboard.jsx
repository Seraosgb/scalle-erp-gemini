...
export default function ProjetoDashboard() {
    ...
    useEffect(() => {
        if (projetoId) {
            ...
            api.get('/projetos/parametros/status').then(res => {
                const stat = res.data?.data || [];
                setParametrosStatus(stat);
            });
    ...
    const alterarStatusProjeto = async (e) => {
        ...
        try {
            // CORREÇÃO AQUI: Garante que estamos a enviar um payload limpo com o UUID.
            await api.patch(`/projetos/${projetoId}/status`, {
                status_projeto_id: novoStatusId
            });
            setModalStatus(false);
            carregarProjeto();
        } catch (error) {
            console.error(error.response);
            alert("Erro ao alterar status. " + (error.response?.data?.message || 'Verifique o console para detalhes.'));
        }
    };
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        setNovoStatusId(projeto.status_projeto_id || (parametrosStatus.length > 0 ? parametrosStatus[0].id : ''));
                                        setModalStatus(true);
                                    }}
