...
import { ArrowLeft, Kanban, DollarSign, Users, PackageCheck, Paperclip, Link as LinkIcon, Trash2 } from 'lucide-react';
...
export default function ProjetoDashboard() {
    ...
    const toggleTimer = async (tarefa, isRunning) => {
        try {
            if (isRunning) {
                ...
                alert('Cronômetro parado e horas apropriadas!');
            } else {
    ...
    // Coleta todas as tarefas do projeto para o select do modal de Blockers
    ...
    return (
        <div className="min-h-screen flex flex-col space-y-4 relative">
            <header className="bg-slate-900 border border-slate-800 shadow-sm rounded-2xl p-5">
                <div className="flex items-center gap-4 border-b border-slate-800 pb-4">
                    <button onClick={() => navigate('/app/projetos')} className="p-2 bg-slate-950 border border-slate-700 hover:border-slate-500 rounded-lg text-slate-400 hover:text-white transition">
                        ...
                        return (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 ${
                                    activeTab === tab.id ? 'bg-indigo-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:text-slate-200'
                                }`}
                            >
                                                    ...
                                                    {(provided, snapshot) => (
                                                        <div
                                                            ...
                                                            className={`bg-slate-950 p-4 rounded-xl border relative group transition-shadow ${
                                                                snapshot.isDragging ? 'border-indigo-500 shadow-xl shadow-indigo-500/20 z-50' : 'border-slate-800 hover:border-indigo-500/50'
                                                            }`}
                                                            style={{
                                                        ...
                                                        >
                                                            <div className="flex justify-between items-start gap-2">
                                                                <h3 className="font-semibold text-slate-200 text-sm leading-tight">{tarefa.titulo}</h3>

                                                                {/* Menu Rápido do Card */}
                                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    <button onClick={() => setModalBlocker(tarefa)} className="text-slate-500 hover:text-rose-400 p-1" title="Adicionar Dependência"><LinkIcon size={14}/></button>
                                                                    <label className="text-slate-500 hover:text-indigo-400 p-1 cursor-pointer" title="Anexar Arquivo">
                                                                        <Paperclip size={14}/>
                                                                ...
                                                                <div className="mt-2 text-[10px] bg-rose-950/40 text-rose-400 border border-rose-900/50 p-1.5 rounded font-bold flex flex-col gap-1">
                                                                    ...
                                                                    {tarefa.dependencias.map(dep => (
                                                                        <div key={dep.id} className="flex justify-between items-center text-rose-300/80 bg-rose-950/40 px-1 rounded">
                                                                            ...
                                                                            <button onClick={() => removerDependencia(tarefa.id, dep.id)} className="hover:text-white"><Trash2 size={10}/></button>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            )}

                                                            {/* Micro-Gestão: Checklists */}
                                                            <div className="mt-3 bg-slate-900/50 rounded-lg p-2 border border-slate-800/80">
                                                                <div className="flex justify-between items-center mb-2">
                                                                    <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Checklist</span>
                                                                    <button onClick={() => adicionarChecklist(tarefa.id)} className="text-indigo-400 hover:bg-indigo-900/30 w-5 h-5 rounded flex items-center justify-center text-xs font-bold transition">+</button>
                                                                </div>
                                                                <div className="space-y-1.5">
                                                                    {tarefa.checklists?.map(item => (
                                                                        <label key={item.id} className="flex items-start gap-2 text-xs text-slate-300 cursor-pointer group">
                                                                            <input
                                                                                type="checkbox"
                                                                            ...
                                                                            />
                                                                            <span className={`transition-all ${item.concluido ? "line-through text-slate-600" : "group-hover:text-white"}`}>
                                                                                {item.descricao}
                                                                            </span>
                                                                        </label>
                                                                    ))}
                                                                ...
                                                                {(() => {
                                                                    ...
                                                                    return (
                                                                        <button
                                                                            ...
                                                                            className={`flex items-center justify-center w-7 h-7 rounded border transition shadow-md cursor-pointer ${
                                                                                isRunning ? 'bg-rose-950/60 border-rose-600 text-rose-400 animate-pulse' : 'bg-emerald-950/40 border-emerald-800 text-emerald-400 hover:bg-emerald-900'
                                                                            }`}
                ...
                <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
                    <form onSubmit={adicionarDependencia} className="bg-slate-900 p-6 rounded-2xl border border-slate-700 w-full max-w-md space-y-4 shadow-2xl">
                        ...
                        <select
                            ...
                            className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg text-white text-sm focus:ring focus:ring-indigo-500/50 outline-none"
                        >
                        ...
                        <div className="flex justify-end gap-2 pt-4">
                            <button type="button" onClick={() => setModalBlocker(null)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm transition">Cancelar</button>
                            <button type="submit" className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-sm font-bold transition shadow-lg shadow-rose-600/20">Bloquear Tarefa</button>
                        </div>
                    </form>
                </div>
            )}
