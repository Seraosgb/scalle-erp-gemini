...
export default function EstoquePage() {
  ...
  // Formulários
  const [formAjuste, setFormAjuste] = useState({ deposito_id: '', item_id: '', novo_saldo: '', motivo: 'Ajuste de Inventário', lote: '', data_validade: '' });
  ...
  const handleSalvarAjuste = async (e) => {
    ...
    try {
      ...
      setFeedback({ tipo: 'sucesso', msg: 'Saldo de inventário ajustado com sucesso!' });
      carregarDados();
    } catch (err) {
  ...
  const handleUploadXml = async (e) => {
    ...
    try {
      ...
      setFeedback({ tipo: 'erro', msg: 'Falha na importação do XML de NF-e.' });
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2.5">
            <Boxes className="h-6 w-6 sm:h-7 sm:w-7 text-indigo-500" />
            Almoxarifado & WMS
          </h1>
